...
namespace Tests.SuspensionTest
{
    ...
    internal class GivenJvIndexIsMinus11
    {
        ...
        public void ThenTheCorrectSuspensionTreatmentIsReturned()
        {
            ...
            Assert.That(upperSuspensionCannotForm, Is.EquivalentTo(new[] { -6 }));
            Assert.That(upperSuspensionNoteOfResolutionIsDissonant, Is.EquivalentTo(new[] { 0, -2, -5, -7 }));
            Assert.That(upperSuspensionIfOnDownbeatMustFormSuspension, Is.EquivalentTo(new[] { -1, -3 }));
            Assert.That(upperSuspensionNoteOfResolutionIsFree, Is.EquivalentTo(new[] { -4 }));
            // Assert.That(upperSuspensionIfOnDownbeatMustFormSuspensionAndNoteOfResolutionIsDissonant, Is.EquivalentTo(new int[] { }));

            Assert.That(lowerSuspensionCannotForm, Is.EquivalentTo(new[] { -1 }));
            Assert.That(lowerSuspensionNoteOfResolutionIsDissonant, Is.EquivalentTo(new[] { 0, -2, -4, -7 }));
            Assert.That(lowerSuspensionIfOnDownbeatMustFormSuspension, Is.EquivalentTo(new[] { -3, -6 }));
            Assert.That(lowerSuspensionNoteOfResolutionIsFree, Is.EquivalentTo(new[] { -5 }));
            // Assert.That(lowerSuspensionIfOnDownbeatMustFormSuspensionAndNoteOfResolutionIsDissonant, Is.EquivalentTo(new int[] { }));
